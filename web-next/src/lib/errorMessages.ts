...
// الأصلي معروف ومتوثّق: verifier قديم فاضل معلّق على نفس عنصر الـDOM من غير clear() قبل ما
// المودال يتقفل ويتفتح تاني — اتصلح فعليًا في clearRecaptchaVerifier()/getRecaptchaVerifier()
// في RegisterForm.tsx (commit 86c8e84، بعد ما لوحة الأدمن سجّلت 143 حالة في 7 أيام).
//
// الرسالة دي رجعت تاني بعد الإصلاح ده (72 حالة في 7 أيام، سبتمبر 2026) — تحقيق منفصل كامل
// (راجع كل مسارات استخدام reCAPTCHA، كل كوميت لمس RegisterForm.tsx من وقت الإصلاح، سيناريو
// الضغط المزدوج، Enter key) استبعد أي regression في كودنا، واستبعد كمان WebView وSafari/iOS
// وتركّز في يوم/متصفح معيّن كأسباب. الخلاصة الأرجح (مش مؤكدة بتوثيق رسمي زي auth/error-code:-39
// تحت، بس أقوى فرضية ناجية من كل الاختبارات): عدم استقرار الشبكة وقت تحميل/تحدي الـreCAPTCHA
// نفسها بيسيب حالة grecaptcha الداخلية معلّقة بشكل مش قابل للتنظيف من جوه .clear() بتاعنا —
// مش باج في كودنا، زي auth/error-code:-39 بالظبط بس في مرحلة مختلفة من الرحلة.
...
export function authErrorMessage(err: any): string {
  ...
  if (message.includes(RECAPTCHA_ALREADY_RENDERED)) {
    return "حصلت مشكلة تقنية مؤقتة في نظام التحقق — جرب تسجّل بالإيميل بدل التليفون (هياخد نفس الوقت تقريبًا)، أو اقفل الصفحة وافتحها تاني وجرب التليفون من الأول";
  }
